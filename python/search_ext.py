...
def search_file(extension):
    ...
    with open('search_result.txt', constants.WRITE, encoding=constants.UTF8) as directory_file:
        for result in results:
            ...
            directory_file.write(f'{split_file_path.split(constants.DOS_SLASH)[-1]}\t{split_file_path.split(constants.DOT)[-1].lower()}\t{result[constants.FILE_SIZE]}\t{split_file_path}\n')
